...
const AsideItem = ({ title, svgName, active, children, md }) => {
	return (
		<li>
			<a
				className={`flex ${
					md
						? 'flex-col  px-8 py-2 my-2 items-center  '
						: 'items-center px-6 py-2 text-sm '
				} hover:bg-gray-100  ${active ? 'bg-gray-200 text-red-500' : ' '}`}
				href='#'
			>
				{children}
				<span className={`${md ? 'text-xs ' : ''}`}>{title}</span>
			</a>
		</li>
	)
}
export const Aside = () => {
	return (
		<>
			<aside className='hidden md:block xl:hidden z-40  border-gray-100 min-h-screen overflow-auto  fixed left-0'>
				<section>
					<ul>
						<AsideItem md title='Главная' active>
							<svg
